...
- ​作用​：Git 钩子工具配置目录，用于在代码提交、推送等操作前自动执行脚本（如代码格式校验、单元测试等）。

​- 常见用途​：自动运行 eslint --fix、prettier --write 或测试用例，确保提交代码质量。

2. .umi/

- 作用​：UmiJS 框架生成的临时文件目录，包含路由缓存、构建中间产物等。

- 注意​：通常不需要手动修改，由框架自动生成和管理。

3. config/

- ​作用​：项目配置文件目录，包含 UmiJS 的核心配置（如路由、代理、插件等）。

- 常见文件​：config.ts（主配置）、routes.ts（路由定义）、env.ts（环境变量）等。

4. mock/

- ​作用​：开发阶段的模拟数据目录，用于模拟后端 API 响应。

- ​常见内容​：JSON 文件或 Mock.js 脚本，提供接口返回的假数据。

5. node_modules/

- 作用​：项目依赖包目录，通过 npm install 或 yarn install 安装的第三方库。

6. public/

- ​作用​：静态资源目录，直接复制到构建输出目录，不经过打包处理。

- ​常见内容​：favicon.ico、全局字体、第三方 CDN 脚本等。

7. .gitignore、.prettierignore 等配置文件

- ​作用​：定义 Git 忽略规则、代码格式化忽略规则等，确保版本控制和代码规范的一致性。
...
- ​作用​：可复用的 UI 组件目录，存放非页面级的通用组件（如按钮、表单、表格等）。

- ​特点​：通常按功能或模块拆分，支持单独测试和复用。

2. locales/

- ​作用​：国际化（i18n）配置目录，存放多语言翻译文本。

- ​常见内容​：不同语言的 JSON 文件（如 zh-CN.ts、en-US.ts），定义界面文案。

3. pages/

- ​作用​：页面级组件目录，UmiJS 中路由与页面组件一一对应（约定式路由）。

​子文件说明​：

- TableList.tsx：表格列表页面
...
- 作用​：API 服务层目录，封装后端接口调用逻辑。

​常见内容​：

- access.ts：权限相关接口（如登录、角色管理）

- requestErrorConfig.ts：请求错误处理配置（如统一错误提示）

其他接口文件（如用户、数据查询等 API 封装）
